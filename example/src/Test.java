import com.inlite.wabr.WABarcode;
import com.inlite.wabr.WABarcodeReader;
import com.inlite.wabr.WAUtils;
import com.inlite.wabr.delReaderCallback;

public class Test
{
	// Image selection
	public boolean bTestDropBox = true;
	public boolean bTestSamplesLocal = true;
	public boolean bTestSamplesWeb = true;
	public boolean bTestBase64 = true;
	public boolean bTestUtf8 = true;
	public boolean bTestUtf8Names = true;

	// Async control
	public boolean bUseAsync = false;

	// Testing
	private boolean bShowDiag = false; // true  for TEST only
	public boolean bShowBarcodes = true; // false  for TEST only
	public int count, errors;

	public delReaderCallback callback = null;



	public String samplesLocal = "";
	public String samplesWeb = "https://wabr.inliteresearch.com/SampleImages/";
	
	public Test()
	{
		if (samplesLocal == "")
			samplesLocal = System.getProperty("user.dir") + "/";
	}

	public final void Run(WABarcodeReader reader, boolean async)
	{
		bUseAsync = async;
		count = errors = 0;
		// Read from image in DropBox
		if (bTestDropBox)
		{
			Read(reader, "https://www.dropbox.com/s/qcd8zfdvckwwdem/img39.pdf?dl=1");
		}

		// Read local files
		if (bTestSamplesLocal)
		{
			Read(reader, samplesLocal + "1d.pdf");
		}

		// Read from Web-located files
		if (bTestSamplesWeb)
		{
			Read(reader, samplesWeb + "1d.pdf");
			// Read 2D barcodes
			Read(reader, samplesWeb + "2d.pdf");
			// Read specifying type
			ReadOpt(reader, samplesWeb + "postal.pdf", "postal", null, 0);
			// Read Driver License (TBR code)
			Read(reader, samplesWeb + "drvlic.ca.jpg");

			// Read from multiple images in a single call
			Read(reader, samplesWeb + "1d.pdf" + " | " + samplesWeb + "2d.pdf");
			// Read from multi-page TIF file
			Read(reader, samplesWeb + "c39.multipage.tif");


			// Read specifying Target Barcode Reader (TBR code)
			ReadOpt(reader, samplesWeb + "I25.tbr132.jpg", null, null, 132);

			// Read AdobeÂ® LiveCycleÂ® barcodes and decompress.  (UTF-8 incorrectly displayed by echo(barcode.Text))
			Read(reader, samplesWeb + "pdf417.adobe.pdf");
		}

		// Read UTF8-encoded values (UTF-8 incorrectly displayed by echo(barcode.Text))
		if (bTestUtf8)
		{
			Read(reader, samplesWeb + "pdf417.utf8.pdf");
		}

		// Read from UTF8-named files
		if (bTestUtf8Names)
		{
			Read(reader, samplesWeb + "pdf417.ЙшзщЪфг_Russian.pdf");
			Read(reader, samplesWeb + "pdf417.بساطة لأنه الأل_Arabic.pdf");
		}

		// Read from Base64-encoded image
		String base64 = "";
		if (bTestBase64)
		{
			Read(reader, base64);
		}

		//  Read from Base64-encoded local image file
		if (bTestBase64)
		{
			base64 = WAUtils.FileToBase64(samplesLocal + "2d.pdf");
			if (!base64.equals(""))
			{
				Read(reader, base64);
			}
		}
	}


	public final void Read(WABarcodeReader reader, String image)
	{
		bShowDiag = reader.diagCallback != null;

		try
		{
			if (bUseAsync)
			{
  			    callback = new ReaderCallback();
				reader.ReadAsync(image, callback, image);
			}
			else
			{
				WABarcode[] barcodes = reader.Read(image);
				if (bShowBarcodes)
					ShowBarcodes(barcodes);
			}
		}
		catch (RuntimeException ex)
		{
			ShowError(ex);
		}
	}


	public final void ReadOpt(WABarcodeReader reader, String image, String types, String directions, int tbt_code)
	{
		bShowDiag = reader.diagCallback != null;

		try
		{
			if (bUseAsync)
			{
				callback = new ReaderCallback();
				reader.ReadOptAsync(image, callback, image, types, directions, tbt_code);
			}
			else
			{
				WABarcode[] barcodes = reader.ReadOpt(image, types, directions, tbt_code);
				if (bShowBarcodes)
					ShowBarcodes(barcodes);
			}
		}
		catch (RuntimeException ex)
		{
			ShowError(ex);
		}
	}

	public static void ShowBarcodes(WABarcode[] barcodes)
	{
		for (WABarcode barcode : barcodes)
		{
			WAUtils.printLine();
			WAUtils.printLine("Barcode Type:" + barcode.getType() + "  File:" + barcode.getFile());
			WAUtils.printLine(barcode.getText());

			if (barcode.getValues().size() > 0)
			{
				WAUtils.printLine("\n VALUES:");
				for (String key : barcode.getValues().keySet())
				{
					WAUtils.printLine(key + " : " + barcode.getValues().get(key));
				}
			}
		}
	}

	public static void ShowError(Exception ex)
	{
		WAUtils.printLine();
		WAUtils.printLine("EXCEPTION: " + ex.getMessage());
	}

	public class DiagCallback implements delReaderCallback {
		@Override

		public void procEvent (String evnt, Object value, Object obj)
		{
			String image = "";
			if (value == null)
			{
				return;
			}
			if (!bShowDiag)
			{
				return;
			}
			switch (evnt)
			{
			case "image":
				image = value.toString();
				WAUtils.printLine("================= PROCESSING:  " + WAUtils.signature(image));
				break;
			case "response":
				image = (String)obj;
				if (value.getClass() == String.class)
				{
					WAUtils.printLine("\n ==== > " + "ERROR: " + value.toString() + WAUtils.signature(image));
				}
				break;
			case "redirect":
				image = value.toString();
				WAUtils.printLine("     ============ REDIRECT: "  + value.toString());
				break;
			}
			return;
		}
	}

	class ReaderCallback implements delReaderCallback {
		@Override
		public void procEvent(String evnt, Object value, Object obj)
		{
			if (value == null)
			{
				return;
			}
			switch (evnt)
			{
			case "error":
				// throw new Exception(value.ToString());  // TEMP TEST - Goes nowhere
				errors++;
				WAUtils.printLine();
				WAUtils.printLine("ERROR: " + value.toString());
				break;
			case "barcodes":
				count++;
				if (bShowBarcodes)
					ShowBarcodes((WABarcode[])value);
				break;
			default:
				delReaderCallback diag = new DiagCallback(); 
				diag.procEvent(evnt, value, obj);
				break;
			}
		}
	}
}